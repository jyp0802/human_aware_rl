...
def gen_population_trainer_from_params(params):
    ...
    def gen_policy():
        lr = np.random.uniform(
            low=population_params["hp_range"]["lr"][0],
            high=population_params["hp_range"]["lr"][1],
            size=None)
        gamma = np.random.uniform(
            low=population_params["hp_range"]["gamma"][0],
            high=population_params["hp_range"]["gamma"][1],
            size=None)
        config = {
            "model" : {
    ...
    population_params["policies"] = policies
    ...
    population_actor = PopulationActor.options(name="pop_act").remote(
        population_params["population_size"], population_params["policies"])
    ray.get(population_actor.set_new_agent_pair.remote())

    population_marl = PopulationMARL(
        population_params["population_size"], population_params["K"], population_params["T_select"], 
        population_params["binomial_n"], population_params["inherit_prob"], 
        population_params["perturb_prob"], population_params["perturb_val"], population_params["hp_range"])
    ...
    class PopulationTrainingCallbacks(DefaultCallbacks):
        ...
        def on_episode_end(self, worker, base_env, policies, episode, **kwargs):
            ...
            pop_actor = ray.util.get_actor("pop_act")
            prev_rating_i = ray.get(pop_actor.get_rating.remote(agt_i_key))
            prev_rating_j = ray.get(pop_actor.get_rating.remote(agt_j_key))
            score_i = score[0]
            score_j = score[1]
            rating_i, rating_j = population_marl.compute_rating(prev_rating_i, prev_rating_j, score_i, score_j)
            ray.get(pop_actor.update_rating.remote(agt_i_key, agt_j_key, rating_i, rating_j, score_i, score_j))
        ...
        def on_train_result(self, trainer, result, **kwargs):
            ...
            population_marl.PBT(trainer)

            pop_actor = ray.util.get_actor("pop_act")
            ray.get(pop_actor.set_new_agent_pair.remote())
